...
<head>
<meta charset="UTF-8">
<title><?=$arcInfo['title']?>-好游快爆APP</title>
...
<div class="artCon">
    <div class="artName"><?=$arcInfo['title']?></div>
    <div class="artInfo">
        <span class="sp1"><?=date('Y-m-d', $arcInfo['time'])?></span>
        <span class="sp3"><?=$arcInfo['author']?></span>
        <span class="sp2" id="read_num">0</span>
        <span class="sp4" id="comment_num">0</span>
    </div>
    <div class="content" id="arcBody">
        <?=$arcInfo['body']?>
    </div>
</div>
<div class="area-tab" id="new_hot">
    <div class="hd">
        <div class="tabmn">
            <span class="on">最新</span>
            <?php if($hotList){?><span>热门</span><?php }?>
        </div>
        <a class="more" href="<?=$zq_url?>">进入专区</a>
    </div>
    <div class="tabbd">
        <dl>
            <?php
			...
			if($newList){
				...
				foreach($newList as $v){
					if(!$k){
						$html .= '<dt><a href="' . $v['url'] . '">';
						$html .= '<div class="img"><img lzimg="1" lz_src="' . $v['pic'] . '" alt="' . $v['title'] . '"></div>';
						$html .= '<div class="con"><p class="tit">' . $v['title']. '</p>';
						$html .= '<p class="info"><span class="fl">' . F::cn_substr($v['descript'], 12, '...') . '</span><span class="fr">' . date('m-d', $v['pubdate']) . '</span></p>';
						$html .= '</div></a></dt>';
					} else {
						$html .= '<dd><a href="' . $v['url'] . '"><span>' . date('m-d', $v['pubdate']) . '</span>' . $v['title'] . '</a></dd>';
					}
					$k ++;
				}
			}
    ...
    <?php if($hotList){?>
    <div class="tabbd" style="display: none;">
        <dl>
            <?php
            $html = '';
            $k = 0;
            foreach($hotList as $v){
                if(!$k){
                    $html .= '<dt><a href="' . $v['url'] . '">';
                    $html .= '<div class="img"><img lz_src="' . $v['pic'] . '" alt="' . $v['title'] . '"></div>';
                    $html .= '<div class="con"><p class="tit">' . $v['title']. '</p>';
                    $html .= '<p class="info"><span class="fl">' . F::cn_substr($v['descript'], 12, '...') . '</span><span class="fr">' . date('m-d', $v['pubdate']) . '</span></p>';
                    $html .= '</div></a></dt>';
                } else {
...
<script>
    ...
    (function(){
        var jsConf = <?=json_encode($jsConf)?>;
        var url = "https://app.4399.cn/apis/news/counter/inc/article/"+jsConf.aid+"/"+jsConf.sign+"";
        $.ajax({
            "url":url,
            dataType:"jsonp",
            "success":function(msg){
                if(msg.code == 100){
                    $("#read_num").html(msg.result);
                }
            }
        });
    })();
